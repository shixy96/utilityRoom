...
var Annodoc = (function($, window, undefined) {
	...
	var parseAnnTextbound = function(line, log) {
		...
		var id = m[1], type = m[2], spanstr = m[3], text = m[4];

		// multiple (start, end) spans separated by semicolons
		var spanarr = normalizeSpace(spanstr).split(';'), spans = [];
	...
	var parseAnnRelation = function(line, log) {
		...
		var id = m[1], type = m[2], a1l = m[3], a1i = m[4], a2l = m[5], a2i = m[6];
	...
	var parseAnnEquiv = function(line, log) {
		...
		var id = m[1], type = m[2], argstr = m[3];
	...
	var parseAnnEvent = function(line, log) {
		...
		var id = m[1], type = m[2], trigger = m[3], argstr = m[4];
	...
	var parseAnnModifier = function(line, log) {
		...
		var id = m[1], type = m[2], target = m[3];
	...
	var parseAnnAttribute = function(line, log) {
		...
		var id = m[1], type = m[2], target = m[3], value = m[4];
	...
	var parseAnnNormalization = function(line, log) {
		...
		var id = m[1], type = m[2], target = m[3], norm_id = m[4], text = m[5];
	...
	var parseAnn = function(ann, log) {
		...
		var textbounds = [], relations = [], events = [], equivs = [], attributes = [], normalizations = [], comments = [], error = false;
		...
		// entity.
		var entities = [], triggers = [], isTrigger = {};
	...
	var parseSd = function(sd, log) {
		...
		// features ("attributes")
		var tokens = sentence_text.split(' '), spans = [], attributes = [], aidseq = 1, offset = 0, error = false;
		sentence_text = '';
		for (var i = 0; i < tokens.length; i++) {
			var text_POS_features = parseSdToken(tokens[i]);
			var text = text_POS_features[0], POS = text_POS_features[1], features = text_POS_features[2];
			...
			if (!text.match(/^\s*$/)) { // skip space-only (e.g. newline)
				...
				if (features) {
					...
					for (var j = 0; j < features.length; j++) {
						...
						attributes.push([ 'A' + aidseq++, m[1], 'T' + (i + 1),
								m[2] ]);
					}
				}
			}
		...
		log('SD parse done: ' + spans.length + ' tokens, ' + relations.length
				+ ' dependencies.');
	...
	var parseConllX = function(conll, log) {
		...
		var ids = [], forms = [], postags = [], heads = [], deprels = [], error = false;
		...
		var spans = [], relations = [], offset = 0;
		for (i = 0; i < ids.length; i++) {
			var length = forms[i].length;
			spans.push([ 'T' + ids[i], postags[i],
					[ [ offset, offset + length ] ] ]);
			offset += length + 1;
			// TODO: handle root
			if (heads[0] === 0) {
	...
	var displayComment = function(evt, target, comment, commentText) {
		...
		/*
		 * slight "tooltip" delay to allow highlights to be seen before the
		 * popup obstructs them.
		 */
		var delay = 250; // ms
	...
	var displaySpanInformation = function(evt, target, spanId, spanType, mods,
			spanText, commentText, commentType, normalizations) {
		...
		comment += ((spanLabel.match(/^\s*$/) ? ''
				: '<span class="comment_type_id_wrapper">'
						...
						+ Util.escapeHTML(spanLabel) + '</span></span>'));
		if (mods.length) {
	...
	var displayArcInformation = function(evt, target, symmetric, arcId,
			originSpanId, originSpanType, role, targetSpanId, targetSpanType,
			commentText) {
		var arcRole = target.attr('data-arc-role');
		// &#8594 == Unicode right arrow
		var arrowStr = '&#8594;';
		var arcDisplayForm = Util.arcDisplayForm(spanTypes, null, arcRole,
				relationTypesHash);
		var comment = "";
		...
		comment += '<span class="comment_type_id_wrapper">'
				+ '<span class="comment_type">'
				+ Util.escapeHTML(arcDisplayForm) + '</span></span>';
		displayComment(evt, target, comment, commentText);
	};
	...
	var makeInputHandler = function(eId, defaultLabel, parse, dispatcher,
			visualization, inputArea, parsedArea, logArea) {
	...
	var sentenceLabelWidth = function(sentenceLabels) {
		...
		if (!sentenceLabels || sentenceLabels.length === 0
				|| sentenceLabels.filter(function(x) {
					return !!x
				}).length === 0) {
			return width;
		}
	...
	var displaySentenceLabels = function(parsed, dispatcher, visualization) {
		if (parsed.sentlabels && parsed.sentlabels.length !== 0
				&& parsed.sentlabels.filter(function(x) {
					return !!x
				}).length !== 0) {
	...
	var buildEmbedding = function(elem, eId, options) {
		var inputTabId = eId + '-2', parsedTabId = eId + '-3', infoTabId = eId
				+ '-4';
		...
		var parsedArea = $('<textarea id="' + eId
				+ '-brat" disabled="disabled" '
				+ 'class="embedded-brat-data"></textarea>');
		...
		// tab headers (li+a)
		tabDiv.append([ '<div id="' + eId + '">', '  <ul>',
				'    <li><a href="#' + inputTabId + '">input</a></li>',
				'    <li><a href="#' + parsedTabId + '">brat</a></li>',
				'    <li><a href="#' + infoTabId + '">info</a></li>',
				'  </ul>', '</div>' ].join('\n'));
		// tab content (divs)
		var inputTab = $('<div id="' + inputTabId + '"></div>'), parsedTab = $('<div id="'
				+ parsedTabId + '"></div>'), logTab = $('<div id="' + infoTabId
				+ '"></div>');
	...
	var embedAnnotation = function(elem, parse, cdata, data, options) {
		...
		Util.embed(visId, $.extend({
			'collection' : null
		}, cdata), $.extend({}, parsed), webFontURLs, dispatcher,
				visualizerOptions);
		...
		var inputHandler = makeInputHandler(eId, '' + myIdSeq, parse,
				dispatcher, visualization, inputArea, parsedArea, logArea);
	...
	var resolveEmbeddedReference = function(elem, data) {
		var refId = elem.attr('href'), refElem = $(refId);
	...
	var findBestDocumentMatch = function(s, collections) {
		...
		for (var ci = 0; ci < 2; ci++) {
			...
			if ((d = findMatchingTitle(s, currentColl, currentDocs, matchCase)) !== null) {
			...
			$
					.each(collections,
							function(coll, docs) {
								if ((d = findMatchingTitle(s, coll, docs,
										matchCase)) !== null) {
									matches.push([ coll, d ]);
								}
							});
			...
			for (var i = 0; i < matches.length; i++) {
				var coll = matches[i][0], maxn = Math.max(currentColl.length,
						coll.length), n = 0;
	...
	var createDocumentationLink = function(e, collections) {
		...
		if (collDocByName[s] !== null) {
			var coll = collDocByName[s][0], doc = collDocByName[s][1], url = doc.url;
	...
	var findParseFunction = function(element) {
		var classes = element.className.split(' '), parseFunction;
	...
	var embedFirstPending = function() {
		...
		var element = pending[0], parseFunction = pending[1], collectionData = pending[2];
	...
	var createEmbeddedVisualizations = function(collectionData) {
		...
		$(embedClasses.join(','))
				.each(
						function(idx) {
							// (see http://stackoverflow.com/a/122704 for
							// extend())
							var element = $(this), parseFunction = findParseFunction(this), cdataCopy = jQuery
									.extend(true, {}, collectionData);
							// hide to minimize "jumping" on draw
							// (https://github.com/spyysalo/annodoc/issues/11)
							element.hide();
							pendingEmbeddings.push([ element, parseFunction,
									cdataCopy ]);
						});
