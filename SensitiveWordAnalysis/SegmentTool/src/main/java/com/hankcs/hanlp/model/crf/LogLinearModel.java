...
public class LogLinearModel extends LinearModel {
	...
	public LogLinearModel(String modelFile) throws IOException {
		...
		if (!((HanLP.Config.IOAdapter == null || HanLP.Config.IOAdapter instanceof FileIOAdapter)
				&& !IOUtil.isFileExisted(binPath))) {
			try {
...
//                    assert id == sizeOfTagSet * sizeOfTagSet + (fid - sizeOfTagSet - 1) * sizeOfTagSet;
	...
	private TaskType guessModelType(TagSet tagSet) {
		if (tagSet.size() == 4 && tagSet.idOf("B") != -1 && tagSet.idOf("M") != -1 && tagSet.idOf("E") != -1
				&& tagSet.idOf("S") != -1) {
			return TaskType.CWS;
		}
		if (tagSet.idOf("O") != -1) {
