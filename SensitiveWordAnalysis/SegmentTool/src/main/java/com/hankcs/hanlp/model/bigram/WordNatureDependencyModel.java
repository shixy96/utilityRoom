...
public class WordNatureDependencyModel {
	...
	public WordNatureDependencyModel(String path) {
		...
		if (load(path)) {
			...
			throw new IllegalArgumentException(
					"加载依存句法生成模型" + path + "失败，耗时：" + (System.currentTimeMillis() - start) + " ms");
		}
	}
	...
	public Edge getEdge(Node from, Node to) {
		...
		if (attribute == null)
			attribute = get(from.compiledWord, WordNatureWeightModelMaker.wrapTag(to.label));
		if (attribute == null)
			attribute = get(WordNatureWeightModelMaker.wrapTag(from.label), to.compiledWord);
		if (attribute == null)
			attribute = get(WordNatureWeightModelMaker.wrapTag(from.label),
					WordNatureWeightModelMaker.wrapTag(to.label));
		if (attribute == null) {
