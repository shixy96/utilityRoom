...
class Word2VecTraining {
				...
				while (true) {
					if (word_count - last_word_count > 10000) {
						...
						if (config.getCallback() == null) {
							System.err.printf("%cAlpha: %f  iter: %d  Progress: %.2f%%  Words/thread/sec: %.2fk", 13,
									alpha, local_iter, percent * 100, wordCountActual / (float) (cost_time));
							...
							if (negative > 0)
								for (d = 0; d < negative + 1; d++) {
									if (d == 0) {
										...
										if (target == 0)
											target = Math.abs((int) (next_random % (vocabSize - 1) + 1));
										if (target == word)
											continue;
										label = 0;
									}
	...
	public void trainModel() throws IOException {
		...
		System.err.println();
		logger.info(
				String.format("finished training in %s", Utility.humanTime(System.currentTimeMillis() - timeStart)));
		// lose weight
		syn1 = null;
		table = null;
