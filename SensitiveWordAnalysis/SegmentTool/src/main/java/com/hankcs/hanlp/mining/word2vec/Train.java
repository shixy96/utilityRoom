...
public class Train extends AbstractTrainer {
	...
	protected void localUsage() {
		...
		System.err.printf("\nExamples:\n");
		System.err.printf(
				"java %s -input corpus.txt -output vectors.txt -size 200 -window 5 -sample 0.0001 -negative 5 -hs 0 -binary -cbow 1 -iter 3\n\n",
				Train.class.getName());
	}
	...
	void execute(String[] args) throws IOException {
		...
		System.err.printf("Starting training using text file %s\nthreads = %d, iter = %d\n", config.getInputFile(),
				config.getNumThreads(), config.getIter());
		w2v.trainModel();
	}
